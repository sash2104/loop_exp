...
public:
	...
	// my create function
	template <class T1>
	double Magnitude(const Image<T1>& image1);

	template <class T1>
	  void AngleMagnitude(const Image<T1>& image1, array<double, N>& feats);
...
template <class T>
template <class T1>
double Image<T>::Magnitude(const Image<T1>& image1)
{
  double ret = 0;
  const T1*& pData1=image1.data();
  for(int i=0;i<nElements;i++){
    ret += sqrt(pData[i]*pData[i] + pData1[i]*pData1[i]);
  }
  return ret;
}
...
template <class T>
template <class T1>
void Image<T>::AngleMagnitude(const Image<T1>& image1, array<double, N>& feats)
{
  fill(begin(feats), end(feats), 0);
  const T1*& pData1=image1.data();
  for(int i=0;i<nElements;i++){
    double x = pData[i], y = pData1[i];
    // fprintf(stdout, "%f,%f\t", x, y);
    int angle = static_cast<int>(atan2( y, x ) * 180/PI + 360)%360;
    int id = CStochastic::nearestAngle(angle);
    double magnitude = sqrt(x*x + y*y);
    feats[id] += magnitude;
  }
  for(int i = 0 ; i < N ; i++){
    fprintf(stdout, "%.2f\t", feats[i]);
  }
  cout << endl;
  return;
}
